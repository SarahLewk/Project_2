...
function createFeatures(mortgageData) {
  ...
  var mortgage = L.geoJSON(mortgageData,{
    // Give each feature a popup describing the information pertinent to it
    onEachFeature: function(feature, layer){
      layer.bindPopup("<h3 > Days to Liquidate: "+ feature["Days_to_Liquidate"] + 
      "</h3><h3>Address: " + feature.Full_Address);
    },

    pointToLayer: function(feature, latlng){
      return new L.circle(latlng,
      { radius: markerSize(feature["Days to Liquidate"]),
        fillColor: getColor(feature.MainCategoryDesc),
        fillOpacity: .8,
        color: 'grey',
        weight: .5
      })
